...
export class AppComponent implements OnInit {
  ...
  getData() {
    this.taskService.getData().subscribe((data) => {
      this.taskData1 = data;
      this.avgValue = (this.taskData1.length * 100) / 100;
      this.ArrayData.push({
        name: 'Latitude>0',
        data: this.taskData1.filter((e) => parseInt(e.address.geo.lat) > 0),
      });
      this.ArrayData.push({
        name: 'Latitude<0',
        data: this.taskData1.filter((e) => parseInt(e.address.geo.lat) < 0),
      ...
      this.ArrayData.push({
        name: 'Longitude>0',
        data: this.taskData1.filter((e) => parseInt(e.address.geo.lng) > 0),
      });
      this.ArrayData.push({
        name: 'Longitude<0',
        data: this.taskData1.filter((e) => parseInt(e.address.geo.lng) < 0),
